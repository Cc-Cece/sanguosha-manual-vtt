...
it('compensates native enlarge on real cards, generals and identities for the expanded board', () => {
  const game = createFourPlayerPrototype(loadTestCatalog());
  expect(game['main-deck']).toHaveProperty('cardDefaults.enlarge', 9.4);
  expect(game['general-deck']).toHaveProperty('cardDefaults.enlarge', 10.0);
  expect(game['identity-deck']).toHaveProperty('cardDefaults.enlarge', 10.0);
});
