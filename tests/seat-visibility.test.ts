...
it('binds private displays and the shared personal hand to native seat visibility', () => {
  ...
  for (let n = 1; n <= 4; n++) {
    expect(game[`private-zone-${n}`]).toMatchObject({ onlyVisibleForSeat: [`seat-${n}`], linkedToSeat: [`seat-${n}`], childrenPerOwner: true });
    ...
    expect(game[`private-zone-${n}`]).toHaveProperty('onLeave.onlyVisibleForSeat', null);
  }
