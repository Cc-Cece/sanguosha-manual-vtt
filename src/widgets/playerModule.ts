...
export function createPlayerModule(index: number): Widget[] {
  ...
  return [
    ...
    widget(seatId, 'seat', { parent: moduleId, x: 8, y: 7, width: 75, height: 32, index, color: '#6d2922', clickRoutine: safeSeatClickRoutine,
      playerChangeRoutine: [{ func: 'CALL', widget: 'table-controller', routine: 'updateHandCountsRoutine' }] }),
    label(`player-label-${n}`, `☰ 玩家 ${n}`, 85, 10, 72, moduleId),
    widget(`leave-seat-${n}`, 'button', { parent: moduleId, x: 159, y: 9, width: 42, height: 28, text: '离座',
    ...
    widget(`toggle-perspective-${n}`, 'button', { parent: moduleId, x: 203, y: 9, width: 58, height: 28, text: '👁️ 视角',
    ...
    label(`hand-count-title-${n}`, '🃏 手牌', 263, 10, 62, moduleId),
    widget(`hand-count-${n}`, 'label', { parent: moduleId, x: 327, y: 8, width: 93, height: 28, text: 0, movable: false,
