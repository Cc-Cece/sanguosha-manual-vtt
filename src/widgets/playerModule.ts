...
export function createPlayerModule(index: number): Widget[] {
  ...
  return [
    ...
    widget(`toggle-perspective-${n}`, 'button', { parent: moduleId, x: 201, y: 9, width: 36, height: 28, text: '👁️',
      color: '#1a3038', css: { fontSize: '13px', color: '#80d0ff', borderRadius: '6px', border: '1px solid #488098' }, clickRoutine: perspectiveRoutines[index] }),
    label(`hand-count-title-${n}`, '🃏 手牌', 239, 10, 62, moduleId),
    widget(`hand-count-${n}`, 'label', { parent: moduleId, x: 303, y: 8, width: 117, height: 28, text: 0, movable: false,
    ...
    ...Array.from({ length: 10 }, (_, proxy) => widget(`blind-proxy-${n}-${proxy + 1}`, 'basic', { parent: blindId, width: 72, height: 101,
      movable: true, enlarge: 4, faces: [{ objects: [{ type: 'text', x: 4, y: 40, width: 64, height: 20, value: '牌背', color: '#e6c980', fontSize: 14, textAlign: 'center' }],
