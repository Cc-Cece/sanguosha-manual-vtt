...
export function createLibraryTableWidgets(model: ReserveModel): Widget[] {
  ...
  return [
    ...
    widget('import-to-reserve-tray-btn', 'button', {
      ...
      text: '🚀 确认更新并同步托盘', movable: false, layer: 101, color: '#2b5746',
    ...
    label('summary-sidebar-title', '📊 本局备牌状态', 1208, 70, 272, 'reserve-prep-drawer', { layer: 101 }),
    widget('summary-generals-box', 'label', {
      parent: 'reserve-prep-drawer', x: 1210, y: 102, width: 268, height: 118,
      text: `武将总数：${model.generalCardIds.length}\n允许入局：${model.generalCardIds.length}　Ban：0\n托盘待用：0\n游戏中：0\n待归还后移除：0`, movable: false, layer: 101,
      css: { background: '#121c18', color: '#c5e0d4', fontSize: '12px', lineHeight: '21px', textAlign: 'left', border: '1px solid #3b5448', borderRadius: '6px', padding: '6px', whiteSpace: 'pre-line' },
    }),
    widget('summary-extras-box', 'label', {
      parent: 'reserve-prep-drawer', x: 1210, y: 230, width: 268, height: 118,
      text: `扩展牌总数：${model.extraCardIds.length}\n已选择：${model.extraCardIds.length}　未选：0\n托盘待用：0\n游戏中：0\n待归还后移除：0`, movable: false, layer: 101,
      css: { background: '#121c18', color: '#c5e0d4', fontSize: '12px', lineHeight: '21px', textAlign: 'left', border: '1px solid #3b5448', borderRadius: '6px', padding: '6px', whiteSpace: 'pre-line' },
    }),
    widget('summary-current-box', 'label', {
      parent: 'reserve-prep-drawer', x: 1210, y: 358, width: 268, height: 108,
      text: `当前分类：全部武将\n总数：${model.generalCardIds.length}\n允许：${model.generalCardIds.length}　Ban：0\n游戏中：0　待移除：0`, movable: false, layer: 101,
      css: { background: '#121c18', color: '#f3dfb3', fontSize: '12px', lineHeight: '22px', textAlign: 'left', border: '1px solid #806a3f', borderRadius: '6px', padding: '6px', whiteSpace: 'pre-line' },
    }),
    widget('summary-help-box', 'label', {
      parent: 'reserve-prep-drawer', x: 1210, y: 480, width: 268, height: 150,
      text: '安全更新说明\n• 打开面板只召回仍在托盘中的牌\n• 已离开托盘的牌保持原位\n• 取消使用中的牌会标记“待移除”\n• 该牌归还托盘时会自动退出牌组\n• 整桌重置才会强制召回全部牌', movable: false, layer: 101,
      css: { background: '#101916', color: '#9eb8ad', fontSize: '11px', lineHeight: '21px', textAlign: 'left', border: '1px solid #354a40', borderRadius: '6px', padding: '6px', whiteSpace: 'pre-line' },
    }),
    widget('reset-draft-btn', 'button', {
      parent: 'reserve-prep-drawer', x: 1210, y: 650, width: 268, height: 36, text: '🔄 恢复全部默认选择',
