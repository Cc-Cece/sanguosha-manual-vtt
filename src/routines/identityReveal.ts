...
const confirmHandRevealRoutine = (cardId: string) => [
  {
    ...
    header: '查看身份牌？',
    fields: [
      { type: 'text', label: '当前可见区域', value: '个人手牌区' },
      { type: 'text', label: '确认结果', value: '确认后仅当前手牌所有者可见身份牌正面。将身份牌移出手牌前，请先将其盖回。' },
    ],
    block: true,
  },
...
const confirmPublicRevealRoutine = (cardId: string) => [
  {
    ...
    header: '公开身份牌？',
    fields: [
      { type: 'text', label: '当前可见区域', value: `公开或未标记区域（区域标识：\${PROPERTY parent OF ${cardId}}）` },
      { type: 'text', label: '确认结果', value: '确认后身份牌将以共享状态翻到正面，所有能够看到当前区域的玩家都能看到身份。' },
    ],
    block: true,
  },
  { func: 'FLIP', collection: [cardId], face: 1 },
