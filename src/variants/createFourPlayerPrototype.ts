...
function tableWidgets(): Widget[] {
  ...
  return [
    ...
    widget('host-toolbar', 'basic', { x: 450, y: 18, width: 845, height: 54, movable: false, onlyVisibleForSeat: ['seat-1'], linkedToSeat: ['seat-1'],
      color: '#20252be8', css: { border: '2px double #b5965b', borderRadius: '10px', boxShadow: '0 4px 14px #000a' } }),
    label('host-toolbar-title', '👑 玩家 1 布局工具', 8, 15, 140, 'host-toolbar'),
    widget('lock-layout', 'button', { parent: 'host-toolbar', x: 150, y: 9, width: 92, height: 36, text: '🔒 锁定布局', clickRoutine: lockLayoutRoutine }),
    widget('unlock-layout', 'button', { parent: 'host-toolbar', x: 247, y: 9, width: 92, height: 36, text: '🔓 解锁布局', clickRoutine: unlockLayoutRoutine }),
    widget('arrange-layout', 'button', { parent: 'host-toolbar', x: 344, y: 9, width: 92, height: 36, text: '🧹 自动整理', clickRoutine: arrangeLayoutRoutine }),
    widget('toggle-tray', 'button', { parent: 'host-toolbar', x: 441, y: 9, width: 92, height: 36, text: '📦 备牌托盘', clickRoutine: toggleReserveTrayRoutine }),
    widget('collect-shuffle', 'button', { parent: 'host-toolbar', x: 538, y: 9, width: 92, height: 36, text: '🔀 收拢洗牌', clickRoutine: collectAndShuffleRoutine }),
    widget('clear-seats', 'button', { parent: 'host-toolbar', x: 635, y: 9, width: 92, height: 36, text: '👤 重置座位', clickRoutine: clearAllSeatsRoutine }),
    widget('reset-table', 'button', { parent: 'host-toolbar', x: 732, y: 9, width: 96, height: 36, text: '🔄 整桌重置', color: '#74322b', clickRoutine: resetTableRoutine }),
