...
function tableWidgets(): Widget[] {
  ...
  return [
    ...
    widget('host-toolbar', 'basic', { x: 485, y: 18, width: 830, height: 54, movable: false, onlyVisibleForSeat: ['seat-1'], linkedToSeat: ['seat-1'],
      color: '#20252be8', css: { border: '2px solid #b5965b', borderRadius: '9px' } }),
    label('host-toolbar-title', '玩家 1 布局工具', 10, 15, 125, 'host-toolbar'),
    widget('lock-layout', 'button', { parent: 'host-toolbar', x: 140, y: 9, width: 85, height: 36, text: '锁定布局', clickRoutine: lockLayoutRoutine }),
    widget('unlock-layout', 'button', { parent: 'host-toolbar', x: 231, y: 9, width: 85, height: 36, text: '解锁布局', clickRoutine: unlockLayoutRoutine }),
    widget('arrange-layout', 'button', { parent: 'host-toolbar', x: 322, y: 9, width: 85, height: 36, text: '自动整理', clickRoutine: arrangeLayoutRoutine }),
    widget('toggle-tray', 'button', { parent: 'host-toolbar', x: 413, y: 9, width: 95, height: 36, text: '备牌托盘', clickRoutine: toggleReserveTrayRoutine }),
    widget('collect-shuffle', 'button', { parent: 'host-toolbar', x: 514, y: 9, width: 100, height: 36, text: '收拢并洗牌', clickRoutine: collectAndShuffleRoutine }),
    widget('clear-seats', 'button', { parent: 'host-toolbar', x: 620, y: 9, width: 90, height: 36, text: '清理座位', clickRoutine: clearAllSeatsRoutine }),
    widget('reset-table', 'button', { parent: 'host-toolbar', x: 716, y: 9, width: 95, height: 36, text: '整桌重置', color: '#74322b', clickRoutine: resetTableRoutine }),
